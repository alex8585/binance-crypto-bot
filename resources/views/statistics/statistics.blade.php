...
<div>
	...
	href="{{ route('statistics',['id'=>$circle_id, 'type'=>'all']) }}" 
	role="button">
		<i class="fa fa-long-arrow-alt-left"></i>
		{{_('Все')}}
	</a> 
	<a style="margin-bottom:15px; margin-right:10px" class="btn btn-secondary back-button" 
	href="{{ route('statistics',['id'=>$circle_id, 'type'=>'with_tv']) }}" 
	role="button">
		<i class="fa fa-long-arrow-alt-left"></i>
		{{_('С TV Statistics')}}
	</a> 
...
<table class="table ">
	...
	@endphp
		<h2 style="display: inline-block;margin-right:20px">{{__('Statistics')}}</h2>
	<thead>
		<tr>
		...
		<tr>
			...
			<td> 
				@isset($e->before_tehnical_1m_new)
					<div> 
						<b><span >1h </span>
						{{ $e->before_tehnical_1m_new->created_at->format('d-m-Y H:i') }}</b>
					</div>
				@endisset
				@isset($e->before_tehnical_5m_new)
					<div> 
						<b><span>5m </span>
						{{ $e->before_tehnical_5m_new->created_at->format('d-m-Y H:i') }}</b>
					</div>
				@endisset
			</td>
			<td> 
				@isset($e->after_tehnical1h_new)
					<div> 
						<span>1h </span>
						<b>{{ $e->after_tehnical1h_new->created_at->format('d-m-Y H:i') }}</b>
					</div>
				@endisset
				@isset($e->after_tehnical5m_new)
					<div> 
						<span>5m </span>
						<b>{{ $e->after_tehnical5m_new->created_at->format('d-m-Y H:i') }}</b>
					</div>
				@endisset
			</td>
			<td>
